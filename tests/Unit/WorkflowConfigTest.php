...
class WorkflowConfigTest extends TestCase
{
    ...
    public function testAddNodeWithCustomTransitions(): void
    {
        ...
        $config->addNode(ConfigTestHandler::class, [
            WorkflowResult::ON_SUCCESS => ConfigTestSuccessHandler::class,
            WorkflowResult::ON_FAIL => ConfigTestFailHandler::class,
            WorkflowResult::ON_SKIP => ConfigTestHandler::class,
            WorkflowResult::ON_EVENT => ConfigTestSuccessHandler::class,
        ]);
        ...
        $this->assertSame(ConfigTestSuccessHandler::class, $transitions[WorkflowResult::ON_SUCCESS]);
        $this->assertSame(ConfigTestFailHandler::class, $transitions[WorkflowResult::ON_FAIL]);
        $this->assertSame(ConfigTestHandler::class, $transitions[WorkflowResult::ON_SKIP]);
        $this->assertSame(ConfigTestSuccessHandler::class, $transitions[WorkflowResult::ON_EVENT]);
    }
    ...
    public function testIsStrictRoutingDefaultsToFalse(): void
    {
        $config = new WorkflowConfig();

        $this->assertFalse($config->isStrictRouting());
    }

    public function testIsStrictRoutingReturnsExplicitConstructorValue(): void
    {
        $strict = new WorkflowConfig(strictRouting: true);
        $nonStrict = new WorkflowConfig(strictRouting: false);

        $this->assertTrue($strict->isStrictRouting());
        $this->assertFalse($nonStrict->isStrictRouting());
    }
    ...
    public function testGetNodesReturnsFullTransitionData(): void
    {
        ...
        $this->assertSame(ConfigTestHandler::class, $nodes[0]['transitions'][WorkflowResult::ON_SKIP]);
    }
}
